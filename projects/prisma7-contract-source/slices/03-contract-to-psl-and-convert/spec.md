...
```prisma
// use prisma-8
// Converted from prisma/schema.prisma by `prisma contract print`.
```
...
- **Command** `contract print` in `packages/1-framework/3-tooling/cli/src/orm/contract/print.ts`, registered in `family.ts` and `cli.ts`. It requires the configured contract source to be a Prisma 7 source, loads the contract through it, prints, and writes with `publishTextArtifact`. Output path resolution reuses `inferredContractPathFor`. Refusals exit 2 and write nothing.
