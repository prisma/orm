...
Explicit relations map directly, keeping relation names. `onDelete` defaults to `Restrict` when any foreign key field is required and to `SetNull` only when every one is optional, so a composite key that mixes an optional and a required field gets `Restrict`, as Prisma 7.10.0 writes it; `onUpdate` defaults to `Cascade`. Both are always set explicitly. An explicit `SetNull` (`onDelete` or `onUpdate`) over a required foreign key field, or `SetDefault` over a required field with no column default (a client-side generator such as `uuid()` gives the column none), is `PRISMA7_REFERENTIAL_ACTION_UNSUPPORTED` on the `@relation` attribute: Prisma 7.10.0 accepts both (with a warning for `SetNull`) and writes them into the foreign key, but the contract rejects them because the action would fail the first time it runs. The message offers making the fields optional (and the relation field, when it is required) or giving them a column default (`ALTER COLUMN ... DROP NOT NULL` or `SET DEFAULT` on Prisma 7's next migration) or another action (the next migration drops and re-adds the foreign key). Fixtures `referential-action-not-null` and `referential-action-not-null-variants`. A relation field may be optional over fields that are all required, which Prisma 7 accepts without a warning: its foreign key and its relation are those of a required relation (the relation is not nullable, because the foreign key guarantees the related row). A required relation field over an optional field is `PRISMA7_RELATION_UNRESOLVED`, as Prisma 7 rejects it ("At least one of those fields is optional. Hence the relation field must be optional as well."). Fixtures `referential-action-defaults` (verified live against Prisma 7.10.0's SQL) and `relation-nullability`.
...
Every edit a message offers is a Prisma 7 schema change that Prisma 7's next migration applies, so the message says what that migration does, checked with `prisma@7.10.0 migrate diff`. `PRISMA7_UNSUPPORTED_TYPE` for `Unsupported(...)` says that Prisma 7 rejects `@ignore` on such a field ("Fields of type `Unsupported` cannot take an `@ignore` attribute") and that removing the field drops its column, and offers `@@ignore` on the model: an empty migration, but the model disappears from the Prisma 7 client as well as from the contract, and Prisma 7 requires `@ignore` on every relation field that points to it ("must specify the `@ignore` attribute, because the model ... it is pointing to is marked ignored"). Fixture `unsupported-type-model-ignored` loads that edit. Every message that advises adding `@ignore` or `@@ignore` says the field or model also disappears from the Prisma 7 client, and advice to remove `@ignore` says the field appears there again, as Prisma 7.10.0's generated client shows. `PRISMA7_NATIVE_TYPE_UNSUPPORTED` offers `@ignore` first (an empty migration; the field also disappears from the Prisma 7 client) and a type change second (`ALTER COLUMN ... SET DATA TYPE`). For a required field whose column has no default (a client-side generator such as `uuid()` gives none), it adds that neither client can insert rows after `@ignore`: Prisma 7's generated create input no longer holds the field and the column is `NOT NULL`. When `@id`, `@unique`, `@@id`, `@@unique`, `@@index`, or a relation's `fields:` uses the field, it offers no `@ignore`, because Prisma 7 still creates that constraint over an `@ignore`d column (the migration is empty); it says the model cannot use this source until the column type changes (fixture `native-type-rejected-uses`). A key, index, or relation over a field whose type or attributes were already reported reports nothing of its own. `PRISMA7_RELATION_MODE_UNSUPPORTED` says removing `relationMode` or setting `"foreignKeys"` makes the next migration add the foreign keys (`ADD CONSTRAINT ... FOREIGN KEY`), which fails on any row that breaks one.
