...
2. **Slice `playground-scratchpad`** — Linear: _pending_
   - **Outcome:** The playground drops arbitrary schema-path arguments and always opens a fixed gitignored scratch directory seeded with multiple `.prisma` files. The client renders a tab strip; a tab's document opens in Monaco only on first click. The generated config uses a glob. Known and accepted until slice 3 lands (corrected by operator ruling 2026-09-24): the pre-slice-3 language server treats config `inputs` as literal paths, so on the glob config the LSP is entirely silent — no diagnostics for any member, opened or not. The playground deliberately displays this gap; slice 3's glob-aware membership brings the bench to life, opened and unopened tabs alike.
