...
- Provider `mongodb` on the Prisma 7 path is `CLI.INIT_PRISMA7_MONGO_UNSUPPORTED`; any other non-Postgres provider is `CLI.INIT_PRISMA7_PROVIDER_UNSUPPORTED` listing `postgresql`; a path with no `datasource` block is `CLI.INIT_PRISMA7_SCHEMA_INVALID`. All exit 2 before any write. `--target` overrides the provider-derived target; a Postgres `--target` on a Mongo schema is the user's choice and passes through to emit.
...
- Next steps on the Prisma 7 path, in order: set `DATABASE_URL`; `prisma db sign`; move routes one at a time to the client in `src/prisma/db.ts`; after each `prisma7 migrate dev`, run `prisma contract emit` then `prisma db sign`; when `@prisma/client` moved, `prisma7 generate`; at cutover, follow the guide's section 4. The typed `nextActions` carry the same steps.
