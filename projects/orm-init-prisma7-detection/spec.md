...
prisma.config.ts            contract: prisma7Schema('prisma/schema.prisma', { output: 'src/prisma/contract.json' })
...
and the printed next steps are: set `DATABASE_URL`, run `prisma db sign`, move routes to the Prisma 8 client one at a time, re-run `prisma contract emit` and `prisma db sign` after each `prisma7 migrate dev`, and follow the guide's cutover section when the last route has moved.
...
- **The contract source** is `prisma7Schema` from `@prisma/orm-postgres/config`, built by `projects/prisma7-contract-source/` slice 1 and open as [PR #30287](https://github.com/prisma/orm/pull/30287). Init writes the config line; it does not read the schema itself beyond finding the `datasource` block.
...
7. **Next steps are the transition routine.** Set `DATABASE_URL`; `prisma db sign`; move routes one at a time; after each `prisma7 migrate dev`, `prisma contract emit` then `prisma db sign`; at cutover, the guide's phase 4. `prisma7 generate` is listed when `@prisma/client` was moved.

## Transitional-shape constraints

- **Postgres lands first.** A schema whose provider is `mongodb` is refused on the Prisma 7 path with a message saying Mongo support is coming, exit 2, until `@prisma/orm-mongo/config` exports `prisma7Schema` (parallel project slice 2). Other providers are refused with the list of supported ones.
- **The cutover step names the guide, not a command,** until `contract convert` exists. The parallel project changes that line when it lands.
- **The command stays `orm init`.** The unified CLI mounts it as `init`; no new command is added.
- **Nothing in this project may depend on `prisma`, `@prisma/prisma7`, `@prisma/client`, or `@prisma/get-dmmf`.** Detection reads text and evaluates the user's config; the Prisma 7 packages are only ever named as install targets.
...
- [ ] A Prisma 7 config that fails to evaluate, a Mongo provider, an unsupported provider, and a `--from-prisma7-schema` path with no `datasource` block each produce a structured error naming the fix, with nothing written.
...
None. Two points were decided by the orchestrator within Will's "generalise the side-by-side setup" ruling and are flagged for veto in `design-notes.md`: the consent also covers the config rename and the script rewrite, and `@prisma/client` is moved to the Prisma 7 CLI's version.
