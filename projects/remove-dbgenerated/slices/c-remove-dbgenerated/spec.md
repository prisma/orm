...
- `prisma contract convert` has its own default printer, [`psl-print/print-column-default.ts`](../../../../packages/3-targets/3-targets/postgres/src/core/psl-print/print-column-default.ts), which writes `dbgenerated("...")` for every function default other than `now()` and `autoincrement()`. It prints the same `sql` form as `mapDefault`, and the round-trip test in `contract-prisma7/test/convert-roundtrip.test.ts` proves it.
