...
## 9. The TypeScript `sql` tag and `${` — decided

**Decided 2026-09-18 (Will):** the TypeScript `sql` tag resolves `\$` to `$`, so a body containing `${` is written `\${`. PSL keeps exactly two escapes and needs no escape for `${` at all, so the two languages differ for the sequence `\$` alone. The alternative of resolving `\$` in both was rejected as a larger change for a rarer body, and keeping `.defaultSql()` past GA for this one input was rejected outright.
