...
2. **Neutral reference shape.** Mongo writes `{ namespace, model, field }`. The hoisted framework type uses neutral names and SQL is migrated. Rejected: writing model and field names into `table` and `column`. That is exactly the vocabulary leak the framework rules forbid.
