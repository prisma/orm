...
_Parent: `projects/mongo-defaults-codecs-prisma6-source/`. Builds on slice 1 (PR #30396). Outcome: a Mongo author writes `temporal.createdAt()` / `temporal.updatedAt()` and the ORM fills the timestamps on create and on non-empty update, with the same contract, runtime, and error semantics as Postgres._
...
"execution": {
  "executionHash": "…",
  "mutations": { "defaults": [
    { "ref": { "namespace": "app", "model": "Post", "field": "createdAt" }, "onCreate": { "kind": "generator", "id": "timestampNow" } },
    { "ref": { "namespace": "app", "model": "Post", "field": "updatedAt" }, "onCreate": { "kind": "generator", "id": "timestampNow" }, "onUpdate": { "kind": "generator", "id": "timestampNow" } }
  ] }
}
```
...
**Contract.** The Mongo arktype schema gains `execution?: { executionHash, mutations: { defaults: [{ ref: { namespace, model, field }, onCreate?, onUpdate? }] } }` with the SQL generator-id rule and `'+': 'reject'`. `MongoContract<S>` is `Omit<Contract<S>, 'execution'> & { execution?: MongoExecutionSection }`. `executionHash` is `computeExecutionHash({ target, targetFamily, execution })` from the framework, computed by both authoring paths, sorted by namespace, model, field. Absent section means no generators; no existing fixture changes.

**Emitter.** The framework emitter already writes `execution` and `executionHash`; the emitted literal must be typed with the Mongo ref shape, so the emitter's `ContractBase` derivation lets the family hook shape `execution` (a family-neutral hook, used by Mongo now; SQL unchanged). Mongo ORM `CreateInput` makes a field optional when its `ref` has `onCreate`, mirroring `IsOptionalCreateField` in `sql-orm-client/src/types.ts:1265-1304`.
