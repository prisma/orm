...
const db = postgres<Contract>({ url: process.env.DATABASE_URL, contractJson })
await db.connect()
...
`db.transaction(fn, options)` on the Postgres and Supabase clients, `withTransaction(runtime, fn, options)`, and `RuntimeConnection.transaction(options)` take an optional `isolationLevel`: `'readUncommitted'`, `'readCommitted'`, `'repeatableRead'` or `'serializable'`. The runtime passes it unchanged to the driver's `SqlConnection.beginTransaction(options)`. Omitting it begins the transaction at the database default, exactly as before.

```typescript
await db.transaction(async (tx) => { /* ... */ }, { isolationLevel: 'serializable' })
```

The driver owns the SQL. The Postgres driver begins with one statement, `BEGIN ISOLATION LEVEL <LEVEL>`, so no separate `SET TRANSACTION` has to win the race to be the transaction's first statement. A driver whose database cannot apply a requested level rejects with `DRIVER.ISOLATION_LEVEL_UNSUPPORTED` before it begins anything; it never ignores the option. The SQLite driver rejects every requested level, because every SQLite transaction is already serializable and there is nothing to select, and the SQLite client's `db.transaction(fn)` takes no options. When `transaction(options)` rejects, `withTransaction` releases the connection and does not call the callback.
