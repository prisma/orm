...
- **Composition helpers:** live in extensions and compose runtime + target descriptors into app-facing entrypoints (for example, `@internal/postgres` exposing the SQL DSL and ORM lanes from one helper)
...
- **Value space ownership**: the target package owns the codecs, data types, and codec descriptors for its database's values, and the adapter registers them on its runtime descriptor; `@internal/target-postgres` and `@internal/target-mongo` export their `codecs`, `codec-ids`, `data-types` and `codec-types`, and `@internal/adapter-postgres` and `@internal/adapter-mongo` register them
...
The pattern's worked example is the SQL family's domain enum: the family contributes the `enum` entity descriptor, the authoring surface (PSL `enum` block / TS `enumType` + `member`) lowers through it into a domain enum entity plus a storage `StorageValueSet`, and — for `managed` tables, unless the column opts out via `@noCheck` — contract build derives an opaque, wire-named CHECK constraint declared on the table like any other ([ADR 244](<../adrs/ADR 244 - Check constraints are opaque wire-named expressions.md>)); the differ compares it by name and the planner reconciles it. Future target-specific entity kinds (RLS policies, custom functions, views) follow the same recipe.
