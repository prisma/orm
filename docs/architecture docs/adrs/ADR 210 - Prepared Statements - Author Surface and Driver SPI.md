...
Postgres and SQLite facades expose `prepare(declaration, params => ...)`, capturing `db.sql` or `db.orm` lexically. SQL-plan callbacks delegate to runtime and retain `PreparedFor` row/statistics semantics. ORM callbacks return a synchronous `.prepared.all()`, `.prepared.first()` or `.prepared.aggregate(selector, configure?)` description, with aggregate descriptions available on both ungrouped and grouped collections. The facade invokes the callback once, passes only its SQL plan to runtime for one lowering, and composes the prepared statement with the ORM-owned consumer. Runtime continues accepting SQL plans only.

The ORM handle's `query(target, params, options?)` returns the complete terminal result directly: a thenable async row stream for `all`, `Promise<Row | null>` for `first`, `Promise<AggregateResult<Spec>>` for an ungrouped aggregate, or `Promise<Array<GroupKeys & AggregateResult<Spec>>>` for a grouped aggregate. An async wrapper around `query` would assimilate the thenable and erase streaming, so preparation is async but the wrapper returns the consumer's result directly. Each invocation owns its result-processing state and uses the explicit compatible runtime, connection or transaction, never the authoring collection's runtime.

Aggregate preparation invokes the selector and annotation callback once, capturing aggregate aliases and codec/empty-result metadata; grouped preparation also captures the storage-to-model mapper. Each execution creates fresh result objects and, for grouped results, a fresh array. SQL runtime decodes projected values. Ungrouped empty-result fallback values are decoded through their contributed codec whenever needed, not cached as mutable values on the prepared description. Grouped empty input returns `[]`.
...
A SQL-builder callback ends with `.build()`, returning a plan; raw SQL callbacks also return plans. `Row` is derived from that plan's row type. An ORM facade callback ends with `.prepared.all()`, `.prepared.first()` or `.prepared.aggregate(...)`, returning a `Preparable<DbRow, Result>` containing a SQL plan and a required result consumer rather than executing an ordinary terminal. ORM predicates preserve non-nullable scalar placeholders in shorthand, callback, relation/include and terminal filters, including fixed lists of individual operands. ORM comparison factories inspect direct `PreparedParamRef` operands at construction time: declaration nullability selects `isNotDistinctFrom` for equality or `isDistinctFrom` for inequality, including HAVING comparisons. PostgreSQL renders `IS NOT DISTINCT FROM` / `IS DISTINCT FROM`; SQLite renders `IS` / `IS NOT`. Other ORM comparison factories reject nullable prepared operands. SQL-builder comparisons and hand-authored structured ASTs retain their explicit operators; ORM does not normalize them. Raw SQL remains opaque, including interpolation payloads: it is neither rewritten nor rejected based on nullable parameters. Literal-null ORM filters retain their null checks. Root and nested ORM pagination accepts SQL's existing non-nullable numeric expression operands, including distinct and scalar/combine include refinements. First-row terminals replace any earlier limit with `1`, retaining unused-declaration validation for overwritten placeholders. ORM mutation preparation, custom helper preparation and dynamic parameter lists are not part of this read-terminal surface.
