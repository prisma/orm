...
`contract print` cannot write the loaded contract as Prisma 8 PSL that reads back as the same contract, so it writes nothing. The message names what it stopped on. Raised when:

- the configured family cannot print a contract (no meta), or the target's descriptor has no `printPslContract` hook (meta: `targetId`);
- no PSL type in the configured stack produces a column's codec and native type. Add the extension that contributes the type to the config (meta: `coordinate`, `nativeType`, `codecId`);
- a column's literal default has no PSL literal that reads back as the stored value, or a domain enum column defaults to a value that is not a member (meta: `coordinate`, `pslTypeName`);
- a column is generated by a generator that has no PSL default function, generates a value on update other than through a temporal preset, or pairs the wall-clock-now generator with a different generator (meta: `coordinate`, `onCreate`, `onUpdate`);
- a to-one relation has no foreign key behind it (meta: `model`, `field`), or a foreign key has no relation that travels it (meta: `namespaceId`, `table`, `columns`);
- a field's type is a union of types (meta: `coordinate`, `kind`), a field is a dictionary (meta: `coordinate`), or a value-object field uses a codec the Postgres target does not own (meta: `coordinate`, `codecId`);
- a column has its own control policy (meta: `coordinate`, `control`), or a model has an owner (meta: `namespaceId`, `modelName`, `owner`);
- one model name is declared in more than one namespace (meta: `modelName`, `namespaces`), or a domain enum or value object is declared outside the default namespace (meta: `namespaceId`, `names`);
- a table has row-level security enabled but no model, or a policy is on a table with no model or names a role that is not a PSL identifier (meta: `namespaceId`, `table`);
- a namespace holds a storage entity kind other than tables, value sets, native enums, row-level security settings, policies and roles (meta: `namespaceId`, `kind`, `names`), or a storage entry that is not an entity of its kind (meta: `namespaceId`, `kind`, `name`).
