...
1. `emit` on the fixture-bearing packages (`examples/*`, `apps/*`, `sql-builder`, `sql-orm-client`, `e2e-tests`, `integration-tests`) — runs the **contract emitter** and rewrites each `contract.json` + `contract.d.ts`. The `integration-tests` package also runs `test/integration/scripts/emit-fixture-configs.mjs`, which walks `test/ports/`, `test/enum-order-by/`, and `test/sql-builder/fixtures/` and re-emits a `prisma.config.ts` only when a committed `generated/` directory sits beside it; every other config is skipped (it belongs to a non-ported suite, or its own test emits to a temp dir).
...
4. `migrations:regen:examples` (`scripts/regen-example-migrations.mjs`) — for each example migration: re-emits its bookend contracts from the package's `contract.prisma` into the content-addressed store at `migrations/snapshots/<hex>/contract.*` (a package without that copy, as `migration plan` writes them, keeps its committed snapshot instead), then re-runs `tsx migration.ts` to rewrite `ops.json` + `migration.json`. Steps 3 and 4 both write the store through `refreshContractSnapshot` (`scripts/refresh-contract-snapshot.mjs`): the plain `writeContractSnapshot` never rewrites an entry whose hash already exists, so the wrapper compares the entry's `contract.json` and `contract.d.ts` with the fresh emit and replaces the whole entry when either file differs.
