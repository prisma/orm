...
- **2026-09-24 — Final orm-init-prisma7-detection retro:** Record an operator ruling as a quote and confirm its interpretation before asking questions that depend on it (F32). Make a test fake return only what the real surface returns (F33). A spec for a command that edits user files names the project's state after each failure; a plan names the release that ships each dependency to users. Merge `main` before manual QA against published builds made from `main`. Manual QA on a real project and an independent review found five bugs the tests missed.
