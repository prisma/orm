...
function load(schema: string) {
  const schemaPath = join(mkdtempSync(join(tmpdir(), 'prisma7-advice-')), 'schema.prisma');
...
describe('advice for generated values Prisma 8 cannot spell', () => {
  it(
    ...
    async () => {
      const rejected = await load(supportedSchema);
      ...
      const loaded = await load(edited);
