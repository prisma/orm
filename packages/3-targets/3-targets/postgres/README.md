...
- **Contract-to-PSL Printing**: Implements the `printPslContract` descriptor hook, which `prisma contract print` uses: it takes an assembled Postgres contract, from any source, and returns the Prisma 8 PSL document that reads back as that same contract: models, value objects, named types, domain and native enums, keys, indexes, checks, relations, polymorphism, control policies and execution defaults. Anything the PSL language has no form for is refused by name with `CONTRACT.PRINT_UNSUPPORTED`, never dropped. It lives in `src/core/psl-print/` and is the reverse of the `inferPslContract` hook in `src/core/psl-infer/`, whose literal, index and enum-block builders it reuses. Model and field names come from the contract, never re-derived; `@@map` and `@map` appear only where the name the PSL source would derive differs. Uniques print as unique indexes, because that is what a Prisma 7 schema lowers them to, and every index prints explicitly, so each relation declines a backing index of its own. Execution generators join back onto their columns: an id generator prints as its `@default` call, and a wall-clock-now pair prints as the `temporal.*` preset of the column's codec.
