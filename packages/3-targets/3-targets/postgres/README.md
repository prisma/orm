...
- **Contract-to-PSL Printing**: Implements the `printPslContract` descriptor hook, which `prisma contract print` uses. It takes an assembled Postgres contract, from any source, and returns the Prisma 8 PSL document that reads back as the same contract. It writes models, value objects, named types, domain and native enums, keys with their names, indexes, checks, relations, polymorphism, control policies, row-level security (`@@rls`, policies, and roles in `namespace unbound`) and defaults. A column is written as the PSL type that reads back with its codec: the native type's usual type, or else a type constructor the configured stack contributes, such as `TimestamptzString(3)` or an extension's `pgvector.Vector(3)`; the family passes the stack's types to the hook for this. Anything it cannot write so that it reads back the same is refused by name with `CONTRACT.PRINT_UNSUPPORTED` (the full list is in `docs/reference/error-reference.md`); nothing is dropped. It lives in `src/core/psl-print/`, the counterpart of the `inferPslContract` hook in `src/core/psl-infer/`, which writes PSL from a live database; it reuses that hook's literal, index, enum-block and default-mapping builders. Model and field names come from the contract; `@@map` and `@map` appear only where the name the PSL source would derive differs. A unique constraint prints as `@@unique` and a unique index as `@@index(..., unique: true)`; every index prints explicitly, so each relation with a foreign key writes `index: false`. Execution generators join back onto their columns: an id generator prints as its `@default` call, and a wall-clock-now pair prints as the `temporal.*` preset of the column's codec.
