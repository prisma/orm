...
/**
 * `defineConfig` records the directory of the file it runs in when a loader
 * publishes it (ADR 253). A section without one was written as a plain object,
 * or evaluated by a loader that predates the base directory, and its relative
 * paths have no anchor.
 */
function noBaseDirDiagnostic(): Diagnostic {
  return {
    code: 'CONFIG.VALIDATION_FAILED',
    severity: 'error',
    summary: 'Prisma ORM configuration does not record the directory it was written in',
    why: `The ${ORM_CONFIG_SECTION_NAME} section carries no baseDir, so relative paths in it have no anchor. defineConfig from your database facade package records it when a current prisma CLI loads the file.`,
    nextActions: [
      { kind: 'edit-file', label: 'Build the orm section with defineConfig in prisma.config.ts' },
      {
        kind: 'user-choice',
        label: 'Update the prisma CLI if the section already uses defineConfig.',
      },
    ],
  };
}
...
function validate(raw: unknown): SectionValidation<PrismaNextConfig> {
  ...
  if (typeof raw['baseDir'] !== 'string') {
    return { ok: false as const, diagnostics: [noBaseDirDiagnostic()] };
  }
