...
- Build a scope-aware symbol table from the CST, including duplicate-declaration diagnostics, named-type binding resolution, generic-block source reconstruction, and typed block-value interpretation against registered block specs.
...
- Block specs in `src/block-spec/` — `fixedBlock` / `entriesBlock` binders, `InferBlock`, the parser-facing `PslBlockSpecDescriptor` view with `blockSpecFactoryOf`, and `deriveParsedBlocks` for direct interpreter callers that hold only a symbol table; `findBlockDescriptor` in `src/extension-block.ts` looks a keyword up in a descriptor namespace. The shared `jsonValue()` rule reads native JSON-compatible literals from the expression AST.
