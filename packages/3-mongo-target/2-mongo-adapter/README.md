...
- **Codec registration**: The runtime descriptor registers the target's codecs (`buildStandardCodecRegistry` from `@internal/target-mongo/codecs`); the control descriptor contributes the PSL scalar map and points emitted contracts at `@internal/target-mongo/codec-types`. The codecs themselves live in `@internal/target-mongo`.
