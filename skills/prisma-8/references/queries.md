...
3. **Reading `.where(filter).delete()` as "delete everything matching".** `delete()` and `update()` change only the first matching row and return `Row | null`; with a non-unique filter the other rows are silently left in place — for an account-deletion flow, that is user data left behind. Bulk writes are `deleteAll()` / `updateAll()` (lazy: await or iterate the result) or `deleteAndCount()` / `updateAndCount()`. Details in the per-target guide.
